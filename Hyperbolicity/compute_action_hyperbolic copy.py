...
class ActionHyperbolicityCalculator:
    ...
    def set_fixed_models(
        self,
        vla,
        action_head,
        proprio_projector,
        cfg_only_action_head: bool = True
    ):
        print("Setting fixed models for hyperbolicity calculation...")
        ...
        print("✅ Fixed models set for consistent hyperbolicity calculation")
    ...
    def collect_actions(
        ...
        try:
            ...
            if total_collected >= self.max_samples:
                ...
                print(f"✅ Action collection complete! Total collected: {total_collected}")
    ...
    def compute_hyperbolicity(self, step: int) -> Optional[Dict]:
        ...
        try:
            # 合并所有收集的actions
            all_actions = np.vstack(self.collected_actions)  # (total_samples, NUM_ACTIONS_CHUNK, ACTION_DIM)
            ...
            print(f"Computing hyperbolicity for {len(all_actions)} actions at step {step}...")
            
            # 将actions flatten为2D: (total_samples, NUM_ACTIONS_CHUNK * ACTION_DIM)
            ...
            results = {
                ...
                "flattened_dim": NUM_ACTIONS_CHUNK * ACTION_DIM,
                "action_stats": action_stats,
            }
            
            # 保存结果
            self.hyperbolicity_history.append(results)
            self._save_results()  # 启用保存
            
            print(f"✅ Hyperbolicity computed: {mean_delta:.6f} ± {std_delta:.6f}")
    ...
    def should_compute(self, step: int) -> bool:
        ...
        return (step > 0 and 
                step % self.compute_frequency == 0 and 
