...
sys.path.append("/home/ouyangzl/openvla-oft/prismatic/models/action_heads")
...
from compute_action_hyperbolic import create_action_hyperbolicity_calculator, integrate_hyperbolicity_into_training
...
class FinetuneConfig:
    ...
    dist_world_size: Optional[int] = 1
    ...
    # Dataset
    data_root_dir: Path = Path("/mnt/nas_ailab_434/434_dataset/modified_libero_rlds")      # Directory containing RLDS datasets
    ...
    save_freq: int = 50000                          # Checkpoint saving frequency in steps
    save_latest_checkpoint_only: bool = False        # If True, saves only 1 checkpoint, overwriting latest checkpoint
                                                     #   (If False, saves all checkpoints)
    resume: bool = False                            # If True, resumes from checkpoint
    resume_step: Optional[int] = 15000               # (When `resume==True`) Step number that we are resuming from
...
def get_run_id(cfg) -> str:
    ...
    if cfg.run_id_override is not None:
        ...
        elif cfg.use_mean_flow:
            run_id += "--MeanFlow"
        elif cfg.use_convex_flow:
            run_id += "--ConvexFlow"
        elif cfg.use_shortcut_model:
            run_id += "--ShortcutModel"
        elif cfg.use_normalizing_flow:
            run_id += "--NormalizingFlow"
        # === 新增 END ===
...
def run_forward_pass(
    ...
    use_mean_flow,  # 新增这行
    use_convex_flow,  # 新增
    use_shortcut_model,  # 新增
    use_normalizing_flow,  # 新增
    use_proprio,
    use_film,
    num_patches,
    ...
    Args:
        ...
        action_head: Action head module (L1RegressionActionHead, VAEActionHead, DiffusionActionHead)
        noisy_action_projector: Noisy action projector (for diffusion)
        proprio_projector: Proprio projector
    ...
    if use_flow_matching or use_ot_flow_matching or use_mean_flow or use_convex_flow or use_shortcut_model or use_normalizing_flow:  # 修复条件
        ...
        batch_size = batch["actions"].shape[0]  # 修复：使用正确的键
    ...
    if not (use_l1_regression or use_diffusion or use_vae or use_flow_matching or use_ot_flow_matching or use_mean_flow or use_end_to_end_diffusion):  # 修改条件
        loss = output.loss
    ...
    # Compute metrics for continuous action representations (L1 regression | diffusion)
        ...
        if use_convex_flow:
            # === Convex Flow Loss ===
            loss = action_head.module.compute_convex_flow_loss(
                x0, x1, t, actions_hidden_states
            )
            # 推理/评估时采样动作
            with torch.no_grad():
                predicted_actions = run_convex_flow_sampling(
                    vla=vla,
        ...
        if use_shortcut_model:
            # === Shortcut Model Loss ===
            loss = action_head.module.compute_shortcut_loss(
                x0, x1, t, actions_hidden_states
            )
            # 推理/评估时采样动作
            with torch.no_grad():
                predicted_actions = run_shortcut_model_sampling(
                    vla=vla,
                    action_head=action_head,
                    proprio_projector=proprio_projector,
                    batch=batch,
        ...
        if use_normalizing_flow:
            ...
            loss = action_head.module.compute_normalizing_flow_loss(
                x0, x1, t, actions_hidden_states
            )
            # 推理/评估时采样动作
            with torch.no_grad():
                predicted_actions = run_normalizing_flow_sampling(
                    vla=vla,
                    action_head=action_head,
                    proprio_projector=proprio_projector,
                    batch=batch,
        ...
        if use_mean_flow:
            # === Mean Flow Loss ===
            loss = action_head.module.compute_meanflow_loss(
                x0, x1, actions_hidden_states
            )
            # 推理/评估时采样动作
            with torch.no_grad():
                predicted_actions = run_mean_flow_sampling(
                    vla=vla,
                    action_head=action_head,
                    proprio_projector=proprio_projector,
                    batch=batch,
        ...
        if use_vae:
            # Predict action
            recon_actions, mu, logvar = action_head.module.forward(actions_hidden_states)
            # Get VAE loss
            loss = action_head.module.get_vae_loss(
            actions_hidden_states, ground_truth_actions, batch_size, device_id
            )
            predicted_actions = recon_actions
...
def run_convex_flow_sampling(
    ...
    for i in range(num_steps):
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            v = action_head.module.forward(actions_hidden_states, t_emb, x)
            x = x + v * dt
    return x.reshape(actions_shape)

# 新增：Shortcut Model 采样函数
def run_shortcut_model_sampling(
    vla,
    action_head,
    proprio_projector,
    batch,
    ...
    """
    Shortcut Model 采样函数 - 使用快捷模型进行采样
    """
    with torch.autocast("cuda", dtype=torch.bfloat16):
        # VLA前向传播获取隐藏状态
        output = vla(
        ...
        # 使用Shortcut Model的predict_action方法
        predicted_actions = action_head.module.predict_action(actions_hidden_states)
        
    return predicted_actions

# 新增：Normalizing Flow 采样函数
def run_normalizing_flow_sampling(
    vla,
    action_head,
    proprio_projector,
    batch,
    ...
    """
    Normalizing Flow 采样函数 - 使用 normalizing flow 进行采样
    """
    # 初始点x0 ~ N(0, I)
    x = torch.randn(
        size=(batch_size, NUM_ACTIONS_CHUNK, ACTION_DIM),
...
def run_mean_flow_sampling(
    ...
    if current_action_mask is not None:
        current_action_mask = current_action_mask.to(device)
    if next_actions_mask is not None:
        next_actions_mask = next_actions_mask.to(device)
    ...
    with torch.autocast("cuda", dtype=torch.bfloat16):
        ...
        # 确保 actions_hidden_states 在正确的设备上
        actions_hidden_states = actions_hidden_states.to(device_id)
        
        # 使用MeanFlow的predict_action方法
        predicted_actions = action_head.module.predict_action(actions_hidden_states)
...
def log_metrics_to_swanlab(metrics, prefix, step, swanlab) -> None:
    """
...
def save_training_checkpoint(
    ...
    if distributed_state.is_main_process:
        ...
        action_head_types = [
            ...
            getattr(cfg, "use_mean_flow", False),
...
def run_validation(
    ...
    with torch.no_grad():
        for batch in val_dataloader:
            ...
            _, metrics = run_forward_pass(
                    ...
                    use_mean_flow=getattr(cfg, "use_mean_flow", False),
                    use_end_to_end_diffusion=getattr(cfg, "use_end_to_end_diffusion", False),
                    use_convex_flow=getattr(cfg, "use_convex_flow", False),  # 新增
                    use_shortcut_model=getattr(cfg, "use_shortcut_model", False),  # 新增
                    use_normalizing_flow=getattr(cfg, "use_normalizing_flow", False),  # 新增
    ...
    if distributed_state.is_main_process:
        log_metrics_to_swanlab(avg_val_metrics, "VLA Val", log_step, swanlab)
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if not dist.is_initialized():
        init_kwargs = {
            ...
            "init_method": cfg.dist_init_method,
        }
        if cfg.dist_world_size is not None:
            init_kwargs["world_size"] = cfg.dist_world_size
        if cfg.dist_rank is not None:
    ...
    assert not (cfg.use_l1_regression and cfg.use_diffusion and cfg.use_vae), (
        "Cannot do both L1 regression, diffusion, and VAE. Please pick one of them!"
    )
    ...
    if distributed_state.is_main_process:
        swanlab.login("zrVzavwSxtY7Gs0GWo9xV")
        swanlab.init(project=cfg.swanlab_project, name=f"ft+{run_id}", config=asdict(cfg))
    ...
    if cfg.only_action_head:
        # 新添加：冻结 VLA backbone（无论是否使用 LoRA）
        print("Freezing VLA backbone parameters...")
    ...
    # 新增：Convex Flow Action Head 初始化
    if getattr(cfg, "use_convex_flow", False):
    ...
    # 新增：Shortcut Model Action Head 初始化
    if getattr(cfg, "use_shortcut_model", False):
        action_head = init_module(
            ShortcutActionHead,
            "action_head",
            cfg,
    ...
    # 新增：Normalizing Flow Action Head 初始化
    if getattr(cfg, "use_normalizing_flow", False):
        action_head = init_module(
            NormalizingFlowActionHead,
            "action_head",
            cfg,
    ...
    if getattr(cfg, "use_mean_flow", False):
        action_head = init_module(
            MeanFlowActionHead,
            "action_head",
            cfg,
    ...
    if cfg.use_flow_matching or cfg.use_ot_flow_matching or cfg.use_mean_flow or cfg.use_convex_flow or cfg.use_shortcut_model or cfg.use_normalizing_flow:  # 新增
        trainable_params += [param for param in action_head.parameters() if param.requires_grad]
    if cfg.use_proprio:
    ...
    hyperbolicity_calculator = None
    if distributed_state.is_main_process:  # 只在主进程中计算
        hyperbolicity_calculator = create_action_hyperbolicity_calculator(
            save_dir=os.path.join(run_dir, "action_hyperbolicity"),
            max_samples=5000,
            compute_frequency=5000
        )
    ...
    if cfg.use_val_set:
        ...
        # Validation Dataloader Information
        print("Validation Dataset size:", len(val_dataloader.dataset))
        print("Validation Batch size:", val_dataloader.batch_size)
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            # Push Metrics to W&B (every swanlab_log_freq gradient steps)
            log_step = gradient_step_idx if not cfg.resume else cfg.resume_step + gradient_step_idx
            if distributed_state.is_main_process and log_step % cfg.swanlab_log_freq == 0:
                log_metrics_to_swanlab(smoothened_metrics, "VLA Train", log_step, swanlab)
                
            # 集成hyperbolicity计算（只在主进程且有action head时）
            if (hyperbolicity_calculator is not None and 
                action_head is not None and 
                distributed_state.is_main_process):
                
                try:
                    hyperbolicity_result = integrate_hyperbolicity_into_training(
                        calculator=hyperbolicity_calculator,
                        vla=vla,
                        action_head=action_head,
                        proprio_projector=proprio_projector if cfg.use_proprio else None,
                        batch=batch,
                    ...
                    if hyperbolicity_result:
                        print(f"Step {log_step}: Action Hyperbolicity = {hyperbolicity_result['hyperbolicity_mean']:.6f} ± {hyperbolicity_result['hyperbolicity_std']:.6f}")
                        
                        # 额外记录到控制台
                        print(f"  - Samples: {hyperbolicity_result['n_samples']}")
                        print(f"  - Action dim: {hyperbolicity_result['action_dim']}")
                        
                except Exception as e:
                    print(f"Warning: Hyperbolicity calculation failed at step {log_step}: {e}")
            ...
            if distributed_state.is_main_process and gradient_step_idx % cfg.swanlab_log_freq == 0:
